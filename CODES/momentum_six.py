...
for sheet_name in sheet_names:
    ...
    latest_minus_7_year_end = adjust_year_end(latest_year,latest_month,-7)
    

    closing_minus_1 = df[df['Year & Month'] == latest_minus_1_year_end]['Close'].values
    closing_minus_7 = df[df['Year & Month'] == latest_minus_7_year_end]['Close'].values
    
    six_month_price_momentum = (closing_minus_1/closing_minus_7)-1

    df['6 month price momentum'] = np.nan
    df.loc[df['Year & Month'] == latest_year_end,'6 month price momentum'] = six_month_price_momentum
...
for sheet_name in sheet_names:
    ...
    for index,row in filtered_df.iterrows():
        ...
        momentum_value = row['6 month price momentum']
...
for sheet_name in sheet_names:
    ...
    Zscore = (df[df['Year & Month']== latest_year_end]['6 month price momentum'] - mean)/std_dev
...
ZScore_df_6mom  = pd.DataFrame(cleaned_data,columns=['Company Name','ZScore 6 month price momentum'])
ZScore_df_6mom['ZScore 6 month price momentum'] = ZScore_df_6mom['ZScore 6 month price momentum'].clip(lower=-4,upper=4)

# # export to excel
# timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
# new_name = f'6mom_{latest_year_end}_{timestamp}'

# with pd.ExcelWriter("output.xlsx",engine="openpyxl", mode='a') as writer:
#     ZScore_df_6mom.to_excel(writer, index=False, header=True, sheet_name=new_name)
...
print(f'6 month price momentum ZScore Calculation success for {year_end_mon}')